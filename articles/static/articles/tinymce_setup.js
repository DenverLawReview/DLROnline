...
tinyMCE.init({
    ...
    // added options for pasting from word
    // Added by psweet. 2017-02-13
    paste_word_valid_elements: "b,strong,i,em,h1,h2,h3,h4,h5",
    paste_remove_styles_if_webkit: false,
    paste_webkit_styles: 'font-variant-caps',
    paste_retain_style_properties: 'font-variant-caps',
    paste_preprocess: function(plugin, args){
        // Clean up the in-text footnote references.
        console.log(args.content);
        var ftnregex = /<a.*?href=\"#_ftn(\d+)\".*?>\[\d+\]<\/a>/g;
        var ftnsubst = `<sup id="fnref:\$1"><a href="#fn:\$1" rel="footnote">\$1</a></sup>`;
        var content = args.content.replace(ftnregex, ftnsubst);
        // Clean up below-line footnotes
        var ftnrefregex = /<div\s*>\s*<p><a href="#_ftnref(\d+)" title="">.*?<\/a>\s*([\s\S]+?)<\/p>\s*<\/div>/g;
        var ftnrefsubst = `<li class="footnote" id="fn:\$1"><p>\$2<a href="#fnref:\$1" title="return to article"> ↩</a><p></li>`;
        content = content.replace(ftnrefregex, ftnrefsubst);
        // Fix wrap.
        content = content.replace(/<div><br clear=\"all\">\s*<hr.*?>/g,
            `<div class="footnotes"><ol>`);
        content = content.replace(/<\/div>$/g, `</ol></div>`);
